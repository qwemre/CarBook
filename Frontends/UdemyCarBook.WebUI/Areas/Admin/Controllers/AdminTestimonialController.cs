...
namespace UdemyCarBook.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminTestimonialController : Controller
    {
        ...
        public async Task<IActionResult> CreateTestimonial(CreateTestimonailDto createTestimonialDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7292/api/Testimonials", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7292/api/Testimonials?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminTestimonial", new { area = "Admin" });
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var resposenMessage = await client.GetAsync($"https://localhost:7292/api/Testimonials/{id}");
            if (resposenMessage.IsSuccessStatusCode)
            {
                var jsonData = await resposenMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<UpdateTestimonailDto>(jsonData);
                return View(values);
            }
            return View();
        }
        ...
        public async Task<IActionResult> UpdateTestimonial(UpdateTestimonailDto updateTestimonialDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7292/api/Testimonials/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminTestimonial", new { area = "Admin" });
            }
